...
int main(int argc, char* argv[]){
    struct  timeval start_io, start_mem, start_comp;
    struct  timeval end_io, end_mem, end_comp;
    clock_t startio, endio;
    
    unsigned long time_io, time_mem, time_comp;
    gettimeofday(&start_io,NULL);
    startio = clock();
    input(argv[1]);    
    endio = clock();
    float total_time = (float)(endio - startio)/CLOCKS_PER_SEC;

    gettimeofday(&end_io,NULL);
    time_io = 1000000 * (end_io.tv_sec-start_io.tv_sec)+ end_io.tv_usec-start_io.tv_usec;
    ...
    time_mem = 1000000 * (end_mem.tv_sec-start_mem.tv_sec)+ end_mem.tv_usec-start_mem.tv_usec;
    ...
    time_comp = 1000000 * (end_comp.tv_sec-start_comp.tv_sec)+ end_comp.tv_usec-start_comp.tv_usec;
    ...
    output(argv[2]);
    endio = clock();
    total_time += (float)(endio - startio)/CLOCKS_PER_SEC;
    gettimeofday(&end_io,NULL);


    printf("NIO Time: %f", total_time);
    time_io += 1000000 * (end_io.tv_sec-start_io.tv_sec)+ end_io.tv_usec-start_io.tv_usec;
    printf("io time is is %ld\n", time_io);
    printf("memory copy time is %ld\n", time_mem);
